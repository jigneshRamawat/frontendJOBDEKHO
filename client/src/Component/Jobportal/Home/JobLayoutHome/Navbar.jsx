...
const Navbar = () => {
  ...
  const { user, logoutUser } = useContext(AppContext); // Only Job Portal user
      ...
      <div className="max-w-7xl mx-auto px-5">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-5">
            {!user?._id ? (
              <>
                <Button text="Login" variant="outline" to="/login" />
                <Button text="Register" to="/register" />
                <Button text="HRMS" variant="outline" to="/hrms" />
              </>
            ) : (
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 border border-red-200 text-red-600 rounded-full hover:bg-red-50 transition font-medium"
                >
                  <LogOut size={16} />
                  Logout
                </button>
        ...
        {isOpen && (
          <div className="md:hidden pb-5 animate-in slide-in-from-top-2 duration-300">
            <div className="flex flex-col gap-4 pt-4 border-t border-gray-100 mt-2">
              {!user?._id ? (
                <>
                  <Button text="Login" variant="outline" to="/login" onClick={closeMenu} />
                  <Button text="Register" to="/register" onClick={closeMenu} />
                  <Button text="HRMS" variant="outline" to="/hrms" onClick={closeMenu} />
                </>
              ) : (
                <>
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center gap-2 px-4 py-3 border border-red-200 text-red-600 rounded-xl hover:bg-red-50 transition font-medium"
                  >
                    <LogOut size={18} />
                    Logout
                  </button>
